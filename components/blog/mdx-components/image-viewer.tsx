...
function Controls() {
  ...
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20">
      <div
        className="flex items-center gap-2 bg-black/60 backdrop-blur-md rounded-full px-4 py-2 border border-white/10 shadow-xl select-none"
        data-zoom-controls
      >
        <button
          onClick={() => zoomOut()}
...
export function ImageViewer({
  ...
  const pointerRef = useRef({ downX: 0, downY: 0 });
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className="fixed inset-0 z-60 flex items-center justify-center
            ...
            duration-200"
          onPointerDown={(e) => {
            pointerRef.current = { downX: e.clientX, downY: e.clientY };
          }}
          onClick={(e) => {
